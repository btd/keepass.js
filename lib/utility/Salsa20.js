...
const ROUNDS = 20;
const SIGMA_WORDS = [0x61707865, 0x3320646E, 0x79622D32, 0x6B206574];
...
class Salsa20 {
   ...
   */
  setKey(key) {
    if(typeof key !== 'string' && !Array.isArray(key) && !Buffer.isBuffer(key)) {
   ...
   */
  setNonce(nonce) {
    if(typeof nonce !== 'string' && !Array.isArray(nonce) && !Buffer.isBuffer(nonce)) {
  ...
  getBytes(numberOfBytes) {
    errors.typeOf(numberOfBytes, 'number', numberOfBytes);
